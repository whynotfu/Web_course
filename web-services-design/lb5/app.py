...
from auth import check_rights, is_admin, can_view_user, can_edit_user
from visits.views import visits_bp

app = Flask(__name__)
app.secret_key = 'lb5-secret-key-change-in-production'
application = app
    ...
    with sqlite3.connect(DATABASE) as db:
        ...
        if not db.execute('SELECT 1 FROM users LIMIT 1').fetchone():
            ...
            db.execute(
                'INSERT INTO users (login, password_hash, last_name, first_name,'
                ' middle_name, role_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
                ('user01', generate_password_hash('User1234!'),
                 'Петров', 'Пётр', 'Петрович', 2, datetime.now().isoformat())
            )
        db.commit()
...
class User(UserMixin):
    def __init__(self, id, login, password_hash, last_name, first_name,
                 middle_name, role_id, created_at, role_name=None):
        self.id = id
        self.login = login
...
def log_visit():
    ...
    try:
        ...
        db.execute(
            ...
            (request.path, user_id, datetime.now().isoformat())
        )
        db.commit()
    except Exception:
        pass
        ...
        if not errors:
            try:
                ...
                new_role_id = (form_data.get('role_id') or None
                               if current_user.is_admin else row['role_id'])
                db.execute(
